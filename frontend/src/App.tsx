...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-gray-100 flex-col font-sans selection:bg-purple-600 selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {getCartCount() > 0 && (
                <span className="absolute -top-1.5 -right-1.5 h-5 w-5 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full text-[10px] font-bold text-white flex items-center justify-center border-slate-950">
                  {getCartCount()}
                </span>
              )}
            </button>
      ...
      <main className="flex-1">
            ...
            <section className="relative overflow-hidden py-12 border-b border-white/10">
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="border-l-2 border-purple-600 pl-8 py-6 bg-gradient-to-r from-purple-900/10 to-transparent rounded-r-2xl">
                  <span className="inline-flex items-center gap-1.5 py-1 px-3 bg-purple-500/10 text-purple-400 rounded-full text-xs font-semibold border-purple-500/20 mb-3">
                ...
                {/* Trust Badges banner */}
                <div className="pt-8 grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl text-xs text-slate-400">
                  ...
                  {getFilteredProducts().map(prod => (
                    ...
                    >
                      ...
                      <div className="relative aspect-[4/3] overflow-hidden bg-[#050510]">
                        ...
                        {/* Badges */}
                        <div className="absolute top-4 left-4 flex-col gap-1.5">
                          <span className="px-2.5 py-1 bg-[#050510]/80 backdrop-blur-md text-[10px] uppercase font-bold tracking-wider text-slate-300 border-white/10 rounded-md">
                            {prod.category}
                          </span>
                        ...
                        {prod.stock <= 5 && (
                          <span className="absolute bottom-4 right-4 px-2 py-0.5 bg-rose-500/25 border-rose-500/30 backdrop-blur-md rounded text-[10px] text-rose-300 font-bold uppercase tracking-wider">
                            Low Stock
                          </span>
                        )}
                      </div>
                      ...
                      {/* Info body */}
                      <div className="p-6 flex-1 flex-col justify-between space-y-4">
        ...
        {activePage === 'payment-info' && (
          <div className="max-w-3xl mx-auto px-4 py-16 animate-fade-in space-y-8">
            ...
            <div className="bg-white/5 border-white/10 rounded-2xl p-6 md:p-8 space-y-8 backdrop-blur-sm relative overflow-hidden shadow-2xl">
              <div className="absolute top-0 right-0 w-32 h-32 bg-purple-600/5 blur-3xl pointer-events-none" />
              <div className="flex gap-4 items-start relative z-10">
                <div className="h-8 w-8 bg-purple-500/15 text-purple-400 font-display font-bold rounded-lg flex items-center justify-center shrink-0 border-purple-500/25 shadow-lg">
                  1
                </div>
              ...
              <div className="border-t border-white/10 pt-6 relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col md:border-r border-white/10 md:pr-6 justify-between">
                  <div>
                    ...
                    <div className="text-xs text-gray-300 space-y-1.5 font-mono bg-black/40 p-3 rounded-xl border-white/5">
        ...
        {activePage === 'policies' && (
          <div className="max-w-3xl mx-auto px-4 py-16 animate-fade-in space-y-8">
            ...
            <div className="bg-white/5 border-white/10 rounded-2xl p-6 md:p-8 space-y-6 text-sm text-slate-300 leading-relaxed backdrop-blur-sm relative overflow-hidden shadow-2xl">
              <div className="absolute top-0 right-0 w-32 h-32 bg-purple-600/5 blur-3xl pointer-events-none" />
      ...
      <footer className="bg-[#03030d] border-t border-white/10 py-12 mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-white/10 mt-8 pt-6 flex flex-col sm:flex-row justify-between items-center text-[10px] text-slate-600 gap-4">
          <p>©️ 2026 Glow State Peptides. All Rights Reserved. Brisbane, QLD.</p>
    ...
    <div className="min-h-screen bg-[#050510] text-gray-100 flex-col font-sans selection:bg-purple-600 selection:text-white">
      <nav className="sticky top-0 z-40 bg-[#08081a]/90 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium">
            <button onClick={() => { setActivePage('shop'); setSelectedProduct(null); }} className={`transition-all ${activePage === 'shop'? 'text-purple-400 font-semibold' : 'text-slate-400 hover:text-purple-400'}`}>Shop Peptides</button>
            <button onClick={() => setActivePage('calculator')} className={`transition-all ${activePage === 'calculator'? 'text-purple-400 font-semibold' : 'text-slate-400 hover:text-purple-400'}`}>Reconstitution Tool</button>
            <button onClick={() => setActivePage('payment-info')} className={`transition-all ${activePage === 'payment-info'? 'text-purple-400 font-semibold' : 'text-slate-400 hover:text-purple-400'}`}>Payment Steps</button>
            <button onClick={() => setActivePage('policies')} className={`transition-all ${activePage === 'policies'? 'text-purple-400 font-semibold' : 'text-slate-400 hover:text-purple-400'}`}>Refund / Policy</button>
          </div>

          <div className="flex items-center gap-4">
            <button onClick={() => setIsCartOpen(true)} className="relative p-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-purple-500/50 text-slate-300 hover:text-purple-300 transition-all flex items-center gap-2 shadow-xl backdrop-blur-sm" id="cart-trigger">
              <ShoppingCart className="h-4 w-4" />
              <span className="hidden sm:inline text-xs font-semibold">Cart</span>
              {getCartCount() > 0 && (
                <span className="absolute -top-1.5 -right-1.5 h-5 w-5 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full text-[10px] font-bold text-white flex items-center justify-center border-slate-950">
      ...
      <main className="flex-1">
        {activePage === 'home' && (
          <div className="animate-fade-in min-h-[70vh] flex-col items-center justify-center text-center px-4 py-20 space-y-8">
            <Logo size={140} />
            <p className="max-w-xl text-slate-300 text-sm sm:text-base leading-relaxed">Glow State Peptides delivers ultra-pure compounds Australia-wide. Third-party tested. Australian owned and operated.</p>
            <button onClick={() => setActivePage('shop')} className="py-3 px-8 bg-gradient-to-r from-blue-600 to-purple-600 hover:brightness-110 text-white font-bold rounded-xl text-sm transition-all shadow-lg shadow-purple-500/10 cursor-pointer">Enter Shop (18+)</button>
          </div>
        )}
        ...
        {activePage === 'shop' && (
          <div className="animate-fade-in space-y-12 pb-20">
            <section className="relative overflow-hidden py-12 border-b border-white/10">
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="border-l-2 border-purple-600 pl-8 py-6 bg-gradient-to-r from-purple-900/10 to-transparent rounded-r-2xl">
                  <span className="inline-flex items-center gap-1.5 py-1 px-3 bg-purple-500/10 text-purple-400 rounded-full text-xs font-semibold border border-purple-500/20 mb-3"><MapPin className="h-3 w-3" /> Based in Brisbane, Queensland</span>
                  <h1 className="font-display font-extrabold text-3xl sm:text-4xl lg:text-5xl text-white tracking-tight leading-[1.1] max-w-4xl uppercase mb-3">Premium Scientific Peptides For <span className="bg-gradient-to-r from-purple-400 via-indigo-400 to-blue-400 bg-clip-text text-transparent">Advanced Research</span></h1>
                  <p className="text-slate-400 text-sm sm:text-base max-w-3xl leading-relaxed">Glow State Peptides delivers ultra-pure compounds Australia-wide. Third-party tested. Australian owned and operated.</p>
                </div>
              </div>
            </section>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-wrap items-center justify-center gap-2 pb-8 border-b border-white/10">
                {getCategories().map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all border cursor-pointer ${selectedCategory === cat? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white border-transparent shadow-lg shadow-purple-500/15' : 'bg-white/5 border-white/10 text-slate-400 hover:text-white'}`}>{cat}</button>
                ))}
              </div>

              {loading? (
                <div className="text-center py-20 text-slate-400 text-sm"><div className="inline-block h-6 w-6 border-2 border-purple-500 border-t-transparent rounded-full animate-spin mb-3"></div><p>Sterilizing laboratory inventory...</p></div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 pt-10">
                  {getFilteredProducts().map(prod => (
                    <div key={prod.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-xl hover:border-purple-500/40 transition-all duration-300 flex flex-col relative group backdrop-blur-sm">
                      <div className="relative aspect-[4/3] overflow-hidden bg-[#050510]">
                        <img src={prod.image_url} alt={prod.name} className="w-full h-full object-cover group-hover:scale-105 transition-all duration-500" referrerPolicy="no-referrer" />
                        <div className="absolute inset-0 bg-gradient-to-t from-slate-900 to-transparent opacity-80" />
                      </div>
                      <div className="p-6 flex-1 flex-col justify-between space-y-4">
                        <div className="space-y-2">
                          <h3 className="font-display font-bold text-lg text-white group-hover:text-purple-400 transition-colors">{prod.name}</h3>
                          <p className="text-slate-400 text-xs leading-relaxed line-clamp-3">{prod.description}</p>
                        </div>
                        <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                          <div className="flex flex-col">
                            <span className="text-[10px] uppercase tracking-wider text-slate-500">Research Fee</span>
                            <div className="flex items-center gap-2">
                              {prod.is_discounted && prod.discount_price? (<><span className="font-mono text-lg font-bold text-emerald-400">${prod.discount_price.toFixed(2)}</span><span className="font-mono text-xs text-slate-500 line-through">${prod.price.toFixed(2)}</span></>) : (<span className="font-mono text-lg font-bold text-white">${prod.price.toFixed(2)}</span>)}
                              <span className="text-[10px] text-slate-400 uppercase font-bold">AUD</span>
                            </div>
                          </div>
                          <button onClick={() => setSelectedProduct(prod)} className="py-2 px-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:brightness-110 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-md shadow-purple-500/10 cursor-pointer"><span>Add to Cart</span><ChevronRight className="h-3.5 w-3.5" /></button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="max-w-3xl mx-auto px-4 py-16 animate-fade-in space-y-8">
            ...
            <div className="bg-white/5 border-white/10 rounded-2xl p-6 md:p-8 space-y-8 backdrop-blur-sm relative overflow-hidden shadow-2xl">
              <div className="border-t border-white/10 pt-6 relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col md:border-r border-white/10 md:pr-6 justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2"><div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></div><h4 className="text-sm font-bold text-blue-400 uppercase tracking-wider">Bank Transfer Details</h4></div>
                    <p className="text-xs text-slate-400 mb-3">Secure local Queensland bank details for secure nationwide processing:</p>
                    <div className="text-xs text-gray-300 space-y-1.5 font-mono bg-black/40 p-3 rounded-xl border-white/5">
                      <p><span className="text-slate-500">Account Name:</span> {paymentDetails?.account_name || 'Glow State'}</p>
                      <p><span className="text-slate-500">Bank:</span> {paymentDetails?.bank_name || 'Commonwealth Bank'}</p>
      ...
      <footer className="bg-[#03030d] border-t border-white/10 py-12 mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-[10px] text-slate-600">©️ 2026 Glow State Peptides. All Rights Reserved. Brisbane, QLD.</div>
      </footer>

      {selectedProduct && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl relative flex-col md:flex-row max-h-[90vh]">
            <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 z-10 p-2 bg-slate-950/80 border-slate-800 rounded-full text-slate-400 hover:text-white"><X className="h-4 w-4" /></button>
            <div className="w-full md:w-1/2 p-6 md:p-8 overflow-y-auto space-y-6 relative">
              ...
              <button onClick={() => { addToCart(selectedProduct, 1); setSelectedProduct(null); }} className="py-3 px-5 bg-gradient-to-r from-blue-600 to-purple-600 hover:brightness-110 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-2 shadow-lg shadow-purple-500/15 cursor-pointer"><ShoppingBag className="h-4 w-4" /><span>Add to Cart</span></button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-[#050510]/90 backdrop-blur-sm z-50 flex justify-end animate-fade-in" id="cart-drawer">
          <div className="w-full max-w-md bg-[#08081a]/95 border-l border-white/10 flex flex-col justify-between h-full shadow-2xl relative">
            <div className="p-5 border-b border-white/10 flex items-center justify-between relative z-10">
              ...
              <button onClick={() => { setIsCartOpen(false); setCheckoutStep('cart'); }} className="p-1.5 hover:bg-white/5 text-slate-400 hover:text-white rounded-lg transition-all cursor-pointer"><X className="h-5 w-5" /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 relative z-10">
              {checkoutStep === 'cart' && (
                <div className="space-y-5">
                  {cart.map(item => {
                    const price = item.product.is_discounted && item.product.discount_price? item.product.discount_price : item.product.price;
                    return (
                      <div key={item.product.id} className="bg-black/40 border-white/10 rounded-xl p-3 flex justify-between gap-4 text-xs items-center">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => updateCartQty(item.product.id, item.quantity - 1)} className="h-6 w-6 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-white flex items-center justify-center font-bold cursor-pointer">-</button>
                          <span className="font-mono text-white font-bold w-5 text-center">{item.quantity}</span>
                          <button onClick={() => updateCartQty(item.product.id, item.quantity + 1)} className="h-6 w-6 rounded-lg bg-white/5 hover:bg-white/10 border-white/10 text-slate-400 hover:text-white flex items-center justify-center font-bold cursor-pointer">+</button>
                        </div>
                      </div>
                    );
                  })}
                  {cart.length > 0 && (
                    <button onClick={() => setCheckoutStep('details')} className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-purple-600 hover:brightness-110 text-white font-bold rounded-xl text-xs transition-all flex items-center justify-center gap-2 shadow-lg shadow-purple-500/10 cursor-pointer"><span>Checkout</span><ArrowRight className="h-3.5 w-3.5" /></button>
                  )}
                </div>
              )}
              ...
              {checkoutStep === 'details' && (
                <form onSubmit={handlePlaceOrderRequest} className="space-y-5 text-xs text-slate-300">
                  <div className="grid grid-cols-2 gap-3">
                    <button type="button" onClick={() => setPaymentMethod('bank_transfer')} className={`p-3.5 rounded-xl border text-left transition-all ${paymentMethod === 'bank_transfer'? 'bg-gradient-to-r from-blue-600/10 to-purple-600/10 border-purple-500 text-purple-300' : 'bg-black/40 border-white/10 text-slate-400'}`}>Bank Transfer</button>
                    <button type="button" onClick={() => setPaymentMethod('paypal_invoice')} className={`p-3.5 rounded-xl border text-left transition-all ${paymentMethod === 'paypal_invoice'? 'bg-gradient-to-r from-blue-600/10 to-purple-600/10 border-purple-500 text-purple-300' : 'bg-black/40 border-white/10 text-slate-400'}`}>PayPal (+3% fee)</button>
                  </div>
                  ...
                  <div className="pt-4 border-t border-white/10 flex gap-3">
                    <button type="button" onClick={() => setCheckoutStep('cart')} className="py-3 px-4 bg-white/5 hover:bg-white/10 text-slate-300 rounded-xl font-semibold transition-all shrink-0 cursor-pointer">Back</button>
                    <button type="submit" className="flex-1 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:brightness-110 text-white font-bold rounded-xl transition-all cursor-pointer">Place Order</button>
                  </div>
                </form>
              )}
              ...
              {checkoutStep === 'success' && placedOrder && (
                <div className="space-y-6 text-xs text-slate-300 leading-relaxed text-center">
                  <div className="h-12 w-12 bg-emerald-500/10 text-emerald-400 rounded-full flex items-center justify-center mx-auto border-emerald-500/20"><Check className="h-6 w-6" /></div>
                  ...
                  <button onClick={() => { setIsCartOpen(false); setCheckoutStep('cart'); setActivePage('shop'); }} className="py-2.5 px-6 bg-white/5 hover:bg-white/10 text-white rounded-xl font-semibold transition-all w-full cursor-pointer border border-white/10">Return to Research Catalog</button>
                </div>
              )}
            </div>
          </div>
